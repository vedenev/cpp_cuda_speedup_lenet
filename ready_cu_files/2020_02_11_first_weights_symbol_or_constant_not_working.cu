...
__device__ float W_conv1_const[W_conv1_size_const];


extern "C" void set_W_conv1_const(float* values)
{
	cudaMemcpyToSymbol(W_conv1_const, values, W_conv1_size_const * sizeof(float));
}

__global__ void convolutions_relu_tmp(int input_offset, float* features_input, int features_input_size_x, int features_input_size_y, int features_input_n_channels,
	float* features_output, int features_output_size_x, int features_output_size_y, int features_output_n_channels,
	int weights_size_x, int weights_size_y,
	float* biases)
{
	...
	for (int index_input_channel = 0; index_input_channel < features_input_n_channels; index_input_channel++)
	{
		...
		for (int weights_index_y = 0; weights_index_y < weights_size_y; weights_index_y++) {
			for (int weights_index_x = 0; weights_index_x < weights_size_x; weights_index_x++) {
				...
				output_value += features_input[input_1d_index] * W_conv1_const[weights_1d_index];
...
__global__ void convolutions_relu(int input_offset, float* features_input, int features_input_size_x, int features_input_size_y, int features_input_n_channels, 
	float* features_output, int features_output_size_x, int features_output_size_y, int features_output_n_channels,
...
__global__ void convolutions(int input_offset, float* features_input, int features_input_size_x, int features_input_size_y, int features_input_n_channels,
	float* features_output, int features_output_size_x, int features_output_size_y, int features_output_n_channels,
	float* weights, int weights_size_x, int weights_size_y,
	float* biases)
{
...
int main(void)
{
	...
	float* array_cpu_3;
	array_cpu_3 = (float*)malloc(sizeof(float) * W_conv1_size_const);
	FILE* file_id_3;
	char* path_3;
	c_stings_concatinate(weights_dir, "W_conv1.bin", &path_3);
	file_id_3 = fopen(path_3, "rb");
	int n_floats_readed = fread(array_cpu_3, sizeof(float), W_conv1_size_const, file_id_3);
	fclose(file_id_3);
	//if (cudaMemcpyToSymbol(W_conv1, array_cpu_3, W_conv1_size_const * sizeof(float)) != cudaSuccess)
	//if (cudaMemcpyToSymbol("W_conv1", array_cpu_3, W_conv1_size_const * sizeof(float), 0, cudaMemcpyHostToDevice) != cudaSuccess)
	//if (cudaMemcpyToSymbol(W_conv1_const, array_cpu_3, W_conv1_size_const * sizeof(float)) != cudaSuccess)
		//if (cudaMemcpyToSymbol("W_conv1_const", array_cpu_3, W_conv1_size_const * sizeof(float), 0, cudaMemcpyHostToDevice) != cudaSuccess)
	//{
	//	printf("cudaMemcpyToSymbol error\n");
	//}
	//checkCudaErrors(cudaMemcpyToSymbol(W_conv1_const, array_cpu_3, W_conv1_size_const * sizeof(float)));

	set_W_conv1_const(array_cpu_3);

	checkCudaErrors(cudaDeviceSynchronize());

	//cutilSafeCall(cudaMemcpyToSymbol(W_conv1_const, array_cpu_3, W_conv1_size_const * sizeof(float), 0, cudaMemcpyHostToDevice));
	//cudaThreadSynchronize();
	//cudaDeviceSynchronize();
	...
	//load_data_to_array(weights_dir, "W_conv1.bin", &W_conv1, W_conv1_size);
	...
	/*
	float* array_gpu_5;
	if (cudaMalloc((void**)&array_gpu_5, sizeof(float) * W_conv1_size_const) != cudaSuccess)
	{
		std::cout << "Error allocating array_gpu_5\n";
	}

	
	cudaMemcpy(array_gpu_5, array_cpu_3, sizeof(float) * W_conv1_size_const, cudaMemcpyHostToDevice);

	float* accuracy_tmp;
	if (cudaMalloc((void**)&accuracy_tmp, sizeof(float) * 1) != cudaSuccess)
	{
		std::cout << "Error allocating accuracy_tmp\n";
	}
	tmp_check<<<1, 1>>>(array_gpu_5, accuracy_tmp);
	float* accuracy_cpu_tmp;
	accuracy_cpu_tmp = (float*)malloc(sizeof(float) * 1);
	cudaMemcpy(accuracy_cpu_tmp, accuracy_tmp, sizeof(float) * 1, cudaMemcpyDeviceToHost);
	printf("accuracy_cpu_tmp[0] = %.6f\n", accuracy_cpu_tmp[0]);
	*/ 
	...
	load_data_to_array(weights_dir, "b_conv1.bin", &b_conv1, b_conv1_size);
	load_data_to_array(weights_dir, "W_conv2.bin", &W_conv2, W_conv2_size);
	load_data_to_array(weights_dir, "b_conv2.bin", &b_conv2, b_conv2_size);
	...
	for (int sample_count = 0; sample_count < n_samples; sample_count++)
	{
		...
		//convolutions_relu<<<grid_featuremaps_1, threadBlock_featuremaps_1>>>(input_offset, x_val, input_size_x, input_size_y, input_n_channels,
		//	featuremaps_1, featuremaps_1_size_x, featuremaps_1_size_y, featuremaps_1_n_channels,
		//	W_conv1, W_conv1_size_x, W_conv1_size_y,
		//	b_conv1);

		convolutions_relu_tmp<<<grid_featuremaps_1, threadBlock_featuremaps_1>>>(input_offset, x_val, input_size_x, input_size_y, input_n_channels,
			featuremaps_1, featuremaps_1_size_x, featuremaps_1_size_y, featuremaps_1_n_channels,
			W_conv1_size_x, W_conv1_size_y,
			b_conv1);
		...
		convolutions_relu<<<grid_featuremaps_2, threadBlock_featuremaps_2>>>(0, featuremaps_1_pooling, featuremaps_1_pooling_size_x, featuremaps_1_pooling_size_y, featuremaps_1_n_channels,
			featuremaps_2, featuremaps_2_size_x, featuremaps_2_size_y, featuremaps_2_n_channels,
			W_conv2, W_conv2_size_x, W_conv2_size_y,
			b_conv2);
		...
		convolutions_relu<<<grid_featuremaps_3, threadBlock_featuremaps_3>>> (0, featuremaps_2_pooling, featuremaps_2_pooling_size_x, featuremaps_2_pooling_size_y, featuremaps_2_n_channels,
		...
		convolutions<<<grid_featuremaps_4, threadBlock_featuremaps_4>>>(0, featuremaps_3, featuremaps_3_size_x, featuremaps_3_size_y, featuremaps_3_n_channels,
			featuremaps_4, featuremaps_4_size_x, featuremaps_4_size_y, featuremaps_4_n_channels,
			W_conv4, W_conv4_size_x, W_conv4_size_y,
			b_conv4);
