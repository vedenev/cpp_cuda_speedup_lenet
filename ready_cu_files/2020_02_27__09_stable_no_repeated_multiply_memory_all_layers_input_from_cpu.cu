...
int main(void)
{
	...
	FILE* file_id_7;
	char* path_7;
	c_stings_concatinate(weights_dir, "x_val.bin", &path_7);
	file_id_7 = fopen(path_7, "rb");
	int n_floats_readed_7 = fread(x_val_cpu, sizeof(float), x_val_size, file_id_7);
	//std::cout << "n_floats_readed_7 = " << n_floats_readed_7 << "\n";
	fclose(file_id_7);
	free(path_7);
	float* x_val_1_sample;
	if (cudaMalloc((void**)&x_val_1_sample, sizeof(float) * input_size) != cudaSuccess)
	{
		std::cout << "Error allocating GPU for x_val_1_sample\n";
	}
	...
	for (int sample_count = 0; sample_count < n_samples; sample_count++)
	{
		...
		cudaMemcpy(x_val_1_sample, (x_val_cpu + input_offset), sizeof(float) * input_size, cudaMemcpyHostToDevice);
		

		//cudaDeviceSynchronize();

		convolutions_relu_constants_weights_shared_memory<<<grid_featuremaps_1, threadBlock_featuremaps_1>>>(0, x_val_1_sample, input_size_x, input_step, input_n_channels, input_size,
